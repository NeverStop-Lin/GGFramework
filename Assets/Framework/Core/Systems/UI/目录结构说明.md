...
| 文件 | 类型 | 职责 | 状态 |
|------|------|------|------|
| `UIBehaviour.cs` | MonoBehaviour基类 | Pipeline管道、UGUI组件、Unity生命周期、组件绑定 | ✅ 活跃 |
| `UIFactory.cs` | 工厂类 | MonoBehaviour工厂，从Prefab创建UI | ✅ 活跃 |
| `UiLifeCycle.cs` | 数据结构 | UI生命周期返回值（ShowTask/HideTask/Target） | ✅ 活跃 |

**说明**: 
- UIBehaviour是唯一的UI基类，集成了Pipeline、UGUI、Unity生命周期
- 旧的普通类架构已彻底移除
- 命名更简洁，符合现代框架风格
