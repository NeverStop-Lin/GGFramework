...
namespace Zenject.Internal
{
    public static class ZenMenuItems
    {
        ...
        public static void ValidateCurrentScene() { ValidateCurrentSceneInternal(); }
        ...
        static void CreateProjectContextInternal(string absoluteDir)
        {
            ...
            var prefabPath = (Path.Combine(assetPath, ProjectContext.ProjectContextResourcePath) +
                ".prefab").Replace("\\", "/");
...
#else
                var prefabObj =
 PrefabUtility.ReplacePrefab(gameObject, PrefabUtility.CreateEmptyPrefab(prefabPath));
#endif
        ...
        {
            return AddCSharpClassTemplate(friendlyName, defaultFileName, templateStr,
        ...
        )
        {
            var absolutePath = EditorUtility.SaveFilePanel("Choose name for " + friendlyName,
                folderPath, defaultFileName + ".cs", "cs");
